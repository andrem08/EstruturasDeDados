...
void parallel(void* t){
    int tid = (int) t;
    printf("Thread %i começou\n",tid);
    int len = i_y_max/NUM_THREADS;
    double z_x;
    double z_y;
    double z_x_squared;
    double z_y_squared;
    ...
    for(i_y = len*tid; i_y < len* (tid + 1); i_y++){
    ...
    printf(" %i terminei\t len: %i\ti+y: %i\ti_x: %i\n",tid, len, i_y, i_x);
};
void compute_mandelbrot(){

    pthread_t threads [NUM_THREADS];
    for (int i = 0; i < NUM_THREADS; ++i) {
        pthread_create(&threads[i], NULL, (void *(*)(void *)) parallel, (void *) i);
    }
    for (int i = 0; i < NUM_THREADS; ++i) {
